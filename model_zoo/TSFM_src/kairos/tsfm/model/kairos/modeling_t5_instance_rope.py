...
from .tunable_rope_utils import InstanceWiseRotaryEmbedding
...
class T5Attention(nn.Module):
    ...
    def _init_rope(self):
        if hasattr(self.config, 'min_period') and self.config.min_period == 'original_rope_init':
            min_period = 2*torch.pi
            max_period = 2*torch.pi*(10000**(1-2/self.key_value_proj_dim))
        else:
        ...
        elif self.config.position_embedding_type=='instance_wise_rope':
            self.rotary_emb = InstanceWiseRotaryEmbedding(
                    self.key_value_proj_dim,
                    input_feature_dim=self.config.instance_rope_input_feature_dim,
                    min_period=min_period,
                    max_period=max_period,
                    init=self.config.rope_init,
                    scale_method=self.config.scale_method if hasattr(self.config, 'scale_method') else "log",
                )
        else:
            raise ValueError(f"Invalid position embedding type: {self.config.position_embedding_type}")
    ...
    def forward(
        ...
        query_states = query_states.view(batch_size, -1, self.n_heads, self.key_value_proj_dim).transpose(1, 2) # [B, H, L, D]
        ...
        if is_cross_attention and past_key_value is not None and is_updated:
            ...
            key_states = key_states.view(batch_size, -1, self.n_heads, self.key_value_proj_dim).transpose(1, 2) # [B, H, L, D]
            value_states = value_states.view(batch_size, -1, self.n_heads, self.key_value_proj_dim).transpose(1, 2) # [B, H, L, D]
        ...
        if cache_position is not None: 
            position_bias = cache_position.unsqueeze(0)
        # compute scores, equivalent of torch.einsum("bnqd,bnkd->bnqk", query_states, key_states), compatible with onnx op>9
        if not self.config.is_cross_attention_pe and is_cross_attention:
            pass
        else: # only for encoder self attention
            if is_cross_attention and self.config.cross_attention_pe_flip:
                key_states = torch.flip(key_states, dims=[-2]) # cross attention key_states is flipped to ensure the time series index is in order
            
            if self.config.position_embedding_type == 'instance_wise_rope':            
                query_states, key_states = self.rotary_emb(query_states, key_states, s_features)
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        s_features=None,
        freq_embedding=None,
        size=None, # different patch size
    ):
        # Model parallel
