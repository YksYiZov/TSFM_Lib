...
class KairosModelPredictor:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ).to(self.device)  # TODO: 加载具体模型实现，并移动到 self.device
    ...
    def predict(self, test_data_input: List[dict], batch_size: int = None) -> List:
        ...
        for batch in tqdm(
            ...
            model_outputs = self.model(past_target=padded_batch.float(), prediction_length=self.prediction_length, generation=True, preserve_positivity=True, average_with_flipped_input=True)  # TODO: 替换为真实预测结果（形状参考你选用的 Forecast 类型）
            forecast = model_outputs["prediction_outputs"]
            # --------------------------------------------------
            ...
            for i, ts in enumerate(batch):
                ...
                # 取出第 i 条预测: shape (num_samples, H)
                pred_i = forecast[i].detach().cpu().numpy()

                # NaN 检查
                if np.isnan(pred_i).any():
                    raise ValueError(f"[NaN DEBUG] 第 {i}-th 序列预测含 NaN")
