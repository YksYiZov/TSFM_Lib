...
def make_selector_path_builder(
    ...
    def build(selector_name: str, seed: int | None = None, real_order_metric: str | None = None) -> Path:
        ...
        fname = fname.replace(".csv", ".csv" if args.ensemble_mode is None else f"_{args.ensemble_mode}.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ensemble_mode', type=str, default=None, help="对什么特殊后缀的结果进行评估")
    ...
    for current_zoo_num in current_zoo_nums:
        ...
        # 5) TODO:参照四个baseline_selector，添加其他 selector 方法的结果汇总
            myfancy_path = build_sel_path("My_Fancy_Select")
            selector_tasks.append(("My_Fancy_Select", None, myfancy_path, f"MyFancy_z{current_zoo_num}-{args.zoo_total_num}"))
