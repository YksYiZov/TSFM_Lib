...
class Baseline_Select_Predictor:
    ...
    def predict(self, test_data_input: List[dict], dataset_name, model_order: Optional[List[int]] = None) -> List:
        ...
        # preds = self._create_ensemble_weighted(preds_dict, model_order, ensemble_size)
        # preds = self._create_ensemble_median(preds_dict, model_order, ensemble_size)
    ...
    def _create_ensemble(self, preds_dict, model_order, ensemble_size):
        ...
        return preds
    def _rank_based_linear_weights(self, ensemble_size):
        ranks = np.arange(ensemble_size)          # 0,1,2,...
        raw_weights = ensemble_size - ranks        # K, K-1, ..., 1
        weights = raw_weights / raw_weights.sum()
        return weights

    def _create_ensemble_weighted(self, preds_dict, model_order, ensemble_size, weights=None):
        """
        加权平均集成
        weights: list or np.ndarray, shape = (ensemble_size,)
        """
        if weights is None:
            weights = self._rank_based_linear_weights(ensemble_size)
            
        assert len(weights) == ensemble_size
        weights = np.array(weights, dtype=np.float32)
        weights = weights / weights.sum()  # 防止权重未归一化

        ensemble_preds = []
        print(f"Selected Top-{ensemble_size} Models (Weighted) : [", end=' ')
        ...
        # 加权求和
        preds = np.sum(
            ensemble_preds * weights[:, None, None, None],
            axis=0
        )

        return preds

    def _create_ensemble_median(self, preds_dict, model_order, ensemble_size):
        """中位数集成"""
        ensemble_preds = []
        print(f"Selected Top-{ensemble_size} Models (Median) : [", end=' ')
        ...
        # 在模型维度取中位数
        preds = np.median(ensemble_preds, axis=0)
